...
#define READ_BUFFER_SIZE 24576 //RAM size: 1024  2048   4096  8192  16384  24576  32768 
#define MAXOUTPUTSIZE 32 //  Quyết định số MCU (khối hình ảnh nhỏ trong JPEG) được giải mã mỗi lần: 8  16  32 
#define NUMBER_OF_DRAW_BUFFER 8  //Số lượng buffer dùng để lưu ảnh trước khi vẽ lên màn hình: 4  6  8 
...
public:
  bool setup(Stream *input, uint8_t *mjpeg_buf, JPEG_DRAW_CALLBACK *pfnDraw, bool enableMultiTask, bool useBigEndian)
  {
    ...
    if (_enableMultiTask)
    {
      ...
      //xTaskCreatePinnedToCore(drawTask, "drawTask", 1600, &_p, 0, &task, 1);
    }
